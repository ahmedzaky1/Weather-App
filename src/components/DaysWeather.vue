...
export default (await import("vue")).defineComponent({
  ...
  methods: {
    async fetchWeatherData() {
      console.log(this.cityName, "Working");
      const apikey = "48c63522a57ecd2af48aa88228fbc80c";
      ...
      await axios
        ...
        .then((Response) => {
          ...
          const filterdDtat = forecastData
            .map((item) => {
              return {
            ...
            .reduce((acc, item) => {
              if (!acc.some((day) => day.date == (item.date, "day"))) {
                acc.push(item);
              }
              return acc;
            }, [])
            .slice(1, 5);
          console.log(filterdDtat, "working");
          this.forecast = filterdDtat;
          this.loading = false;
        })
        .catch((error) => {
